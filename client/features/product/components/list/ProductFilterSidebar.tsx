...
export default function ProductFilterSidebar({ isShow }: { isShow?: boolean }) {
  const { categories, onChangeFilter, filter } = useProductListContext();

  if (!isShow) return null;

  return (
    <div className="m-3 flex flex-wrap items-end gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      <FilterField
        icon={<Tag size={13} />}
        label="Danh mục"
        name="category"
