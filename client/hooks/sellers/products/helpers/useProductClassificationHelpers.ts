...
import { uploadOneImage } from "@/api/upload.api";
...
export default function useProductClassificationHelpers({
  dispatch,
}: HelperProps) {
  ...
  const [countClassifications, setCountClassifications] = useState<number>(1);
  ...
  const addNewClassificationInput = () => {
    ...
    return setCountClassifications((prev) => {
      if (prev == maxIndex) {
        return prev;
      } else {
        return prev + 1;
  ...
  const removeClassification = (index: number) => {
    if (index == 0) return;
    setCountClassifications((prev) => prev - 1);
  ...
  const onchangeClassification = async (
    ...
    if (name === "img" && files) {
      const file = files[0];
      const upload = await handleLoading(uploadOneImage, file);
      imgUploaded = upload.url;
    }
    ...
    setClassifications((prev) => {
      ...
      if (valueIndexDataset === undefined) {
        ...
        newClassificationValues[valueIndex] = {
          ...(newClassificationValues[valueIndex] ?? {}),
          [name]: name === "img" ? imgUploaded : value,
        };
  ...
  const validateClassification = (
    ...
    classifications.forEach((classification) => {
      ...
      classification.values.forEach((classificationValue) => {
        if (classificationValue.name === "") {
          checked.message = "Tên giá trị đang để trống!";
        }
      });
    });
    return checked;
