...
import { uploadProductImageService } from "@/api/product.api";
...
export default function useProductImageHelpers({ dispatch }: ImageHelperProps) {
  ...
  const addNewImageDetailInput = () => {
    setCountImageDetailInput((prev) => {
      if (imageFile.details[prev - 1]) return prev + 1;
      return prev;
    });
  };
  ...
  const uploadImageFile = async () => {
    ...
    const imageUploaded = await handleLoading(
